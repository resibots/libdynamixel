...
int main(int argc, char **argv) {
  ...
  try {
    ...
    std::vector<std::shared_ptr<BaseServo<Protocol1>>> servos_1 =
        auto_detect<Protocol1>(controller);
    std::vector<std::shared_ptr<BaseServo<Protocol2>>> servos_2 =
        auto_detect<Protocol2>(controller);

    if (servos_1.size()) {
      std::cout << "Detected an " << servos_1[0]->model_name() << " with ID "
                << servos_1[0]->id() << std::endl;
      ...
      for (double angle = M_PI / 2; angle < (3 * M_PI / 2) + 1e-6;
           angle += M_PI / 6) {
      ...
      std::cout << "Detected an " << servos_2[0]->model_name() << " with ID "
                << servos_2[0]->id() << std::endl;
