...
public:
  ...
  pack_instruction(id_t id, instr_t instr,
  ...
  static void unpack_data(const std::vector<uint8_t> &packet, uint32_t &res) {
    if (packet.size() != 4)
      throw errors::UnpackError(2);
    res = (((uint32_t)packet[3]) << 24) | (((uint32_t)packet[2]) << 16) |
          (((uint32_t)packet[1]) << 8) | ((uint32_t)packet[0]);
  }

  static void unpack_data(const std::vector<uint8_t> &packet, int32_t &res) {
    if (packet.size() != 4)
      throw errors::UnpackError(2);
    res = (((int32_t)packet[3]) << 24) | (((int32_t)packet[2]) << 16) |
          (((int32_t)packet[1]) << 8) | ((int32_t)packet[0]);
  }
...
**/
  static bool unpack_status(const std::vector<uint8_t> &packet, id_t &id,
    ...
    uint16_t recv_checksum =
        (((uint16_t)packet.back()) << 8) | packet[packet.size() - 2];
    ...
    if (error != 0) {
      ...
      if (error & 0x80)
        err_message
            << "Device alert. Check Hardware Error field from Control Table; ";
      ...
      throw errors::StatusError(id, 2, error,
                                "Status: error while decoding packet with ID " +
                                    err_message.str());
    }
...
protected:
  static uint16_t _checksum(const std::vector<uint8_t> &packet) {
    if (packet.size() == 0)
      throw errors::Error("Checksum (protocol 2): cannot compute checksum, the "
                          "packet is empty");
