...
public:
  ...
  void open_serial(const std::string &name, int baudrate = B115200) {
    ...
    if (_fd != -1)
      throw errors::Error("error attempting to open device " + name +
                          ": an other connexion is active; call `close serial` "
                          "before opening a new connexion");

    _fd = open(name.c_str(), O_RDWR | O_NOCTTY);
    if (_fd == -1)
      throw errors::Error("error opening device:" + name + " " +
  ...
  // general send
  template <typename T> void send(const InstructionPacket<T> &packet) const {
    if (_fd == -1)
      return;
...
std::cout << std::endl;*/
    ...
    if (ret != packet.size()) {
      ...
      throw errors::Error("Usb2Dynamixel::Send: packet not fully written" +
                          ofs.str());
    }
  }

  // general receive
  template <typename T> bool recv(StatusPacket<T> &status) const {
    if (_fd == -1)
      return false;
