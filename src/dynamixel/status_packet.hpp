...
namespace dynamixel {
template <class Protocol> class StatusPacket {
public:
  ...
  typename Protocol::id_t id() const {
    if (!_valid)
      throw errors::Error(
          "StatusPacket: should be valid before calling member function: `id`");
    return _id;
  }

  const std::vector<uint8_t> &parameters() const {
    if (!_valid)
      throw errors::Error("StatusPacket: should be valid before calling member "
                          "function: `parameters`");
    return _parameters;
  }
...
inline std::ostream &operator<<(std::ostream &os,
                                const StatusPacket<Protocol> &st) {
  return st.print(os);
}
}
