...
#define READ_FIELD(Name)                                                       \
  static InstructionPacket<typename Servo<Model>::protocol_t> get_##Name(      \
      typename Servo<Model>::protocol_t::id_t id) {                            \
    return typename Servo<Model>::read_t(                                      \
        id, Servo<Model>::ct_t::Name,                                          \
        sizeof(typename Servo<Model>::ct_t::Name##_t));                        \
  }                                                                            \
                                                                               \
  static typename Servo<Model>::ct_t::Name##_t parse_##Name(                   \
      typename Servo<Model>::protocol_t::id_t id,                              \
      const StatusPacket<typename Servo<Model>::protocol_t> &st) {             \
    typename Servo<Model>::ct_t::Name##_t res;                                 \
    Servo<Model>::protocol_t::unpack_data(st.parameters(), res);               \
    return res;                                                                \
  }                                                                            \
                                                                               \
  InstructionPacket<typename Servo<Model>::protocol_t> get_##Name()            \
      const override {                                                         \
    return Model::get_##Name(this->_id);                                       \
  }                                                                            \
                                                                               \
  long long int parse_##Name(                                                  \
      const StatusPacket<typename Servo<Model>::protocol_t> &st)               \
      const override {                                                         \
...
#define WRITE_FIELD(Name)                                                      \
  static inline InstructionPacket<typename Servo<Model>::protocol_t>           \
      set_##Name(typename Servo<Model>::protocol_t::id_t id,                   \
                 typename Servo<Model>::ct_t::Name##_t value) {                \
    return typename Servo<Model>::write_t(                                     \
        id, Servo<Model>::ct_t::Name,                                          \
        Servo<Model>::protocol_t::pack_data(value));                           \
  }                                                                            \
                                                                               \
  static inline InstructionPacket<typename Servo<Model>::protocol_t>           \
      reg_##Name(typename Servo<Model>::protocol_t::id_t id,                   \
                 typename Servo<Model>::ct_t::Name##_t value) {                \
    return typename Servo<Model>::reg_write_t(                                 \
        id, Servo<Model>::ct_t::Name,                                          \
        Servo<Model>::protocol_t::pack_data(value));                           \
  }                                                                            \
                                                                               \
  inline InstructionPacket<typename Servo<Model>::protocol_t> set_##Name(      \
      typename Servo<Model>::ct_t::Name##_t value) const {                     \
    return Model::set_##Name(this->_id, value);                                \
  }                                                                            \
                                                                               \
  inline InstructionPacket<typename Servo<Model>::protocol_t> reg_##Name(      \
      typename Servo<Model>::ct_t::Name##_t value) const {                     \
    return Model::reg_##Name(this->_id, value);                                \
  }                                                                            \
                                                                               \
  inline InstructionPacket<typename Servo<Model>::protocol_t> set_##Name(      \
      long long int value) const override {                                    \
    return this->set_##Name(                                                   \
        static_cast<typename Servo<Model>::ct_t::Name##_t>(value));            \
  }                                                                            \
                                                                               \
  inline InstructionPacket<typename Servo<Model>::protocol_t> reg_##Name(      \
      long long int value) const override {                                    \
    return this->reg_##Name(                                                   \
        static_cast<typename Servo<Model>::ct_t::Name##_t>(value));            \
  }
...
public:
  ...
  long long int id() const override { return this->_id; }
  ...
  ping(typename Servo<Model>::protocol_t::id_t id) {
    return ping_t(id);
  }
  ...
  static inline InstructionPacket<protocol_t>
  set_goal_position_angle(typename Servo<Model>::protocol_t::id_t id,
                          double rad) {
    double deg = rad * 57.2958;
    if (!(deg >= ct_t::min_goal_angle_deg && deg <= ct_t::max_goal_angle_deg))
      throw errors::ServoLimitError(id, ct_t::min_goal_angle_deg,
                                    ct_t::max_goal_angle_deg, deg);
    typename ct_t::goal_position_t pos =
        ((deg - ct_t::min_goal_angle_deg) *
         (ct_t::max_goal_position - ct_t::min_goal_position) /
         (ct_t::max_goal_angle_deg - ct_t::min_goal_angle_deg)) +
        ct_t::min_goal_position;
    return set_goal_position(id, pos);
  }

  static inline InstructionPacket<protocol_t>
  reg_goal_position_angle(typename Servo<Model>::protocol_t::id_t id,
                          double rad) {
    double deg = rad * 57.2958;
    if (!(deg >= ct_t::min_goal_angle_deg && deg <= ct_t::max_goal_angle_deg))
      throw errors::ServoLimitError(id, ct_t::min_goal_angle_deg,
  ...
  InstructionPacket<protocol_t>
  set_goal_position_angle(double rad) const override {
    return Model::set_goal_position_angle(this->_id, rad);
  }

  InstructionPacket<protocol_t>
  reg_goal_position_angle(double rad) const override {
    return Model::reg_goal_position_angle(this->_id, rad);
  }

  static InstructionPacket<typename Servo<Model>::protocol_t>
  get_present_position_angle(typename Servo<Model>::protocol_t::id_t id) {
    return get_present_position(id);
  }

  InstructionPacket<typename Servo<Model>::protocol_t>
  get_present_position_angle() const override {
    return Model::get_present_position_angle(this->_id);
  }

  static double parse_present_position_angle(
      typename Servo<Model>::protocol_t::id_t id,
      const StatusPacket<typename Servo<Model>::protocol_t> &st) {
    typename Servo<Model>::ct_t::present_position_t pos;
    Servo<Model>::protocol_t::unpack_data(st.parameters(), pos);
    double deg = ((pos - ct_t::min_goal_position) *
                  (ct_t::max_goal_angle_deg - ct_t::min_goal_angle_deg) /
                  (ct_t::max_goal_position - ct_t::min_goal_position)) +
                 ct_t::min_goal_angle_deg;
    double rad = deg / 57.2958;
    return rad;
  }

  double parse_present_position_angle(
      const StatusPacket<typename Servo<Model>::protocol_t> &st)
      const override {
    return Model::parse_present_position_angle(this->_id, st);
  }

  // Bulk operations. Only works if the models are known and they are all the
  // same
  template <typename Id, typename Pos>
  static InstructionPacket<protocol_t>
  set_goal_positions(const std::vector<Id> &ids, const std::vector<Pos> &pos) {
    ...
    return sync_write_t(ct_t::goal_position,
                        _get_typed<typename protocol_t::id_t>(ids), packed);
  }

  template <typename Id, typename Speed>
  static InstructionPacket<protocol_t>
  set_moving_speeds(const std::vector<Id> &ids,
    ...
    for (size_t i = 0; i < speeds.size(); i++)
      packed[i] =
          protocol_t::pack_data((typename ct_t::moving_speed_t)speeds[i]);

    return sync_write_t(ct_t::moving_speed,
                        _get_typed<typename protocol_t::id_t>(ids), packed);
  }

  template <typename Id, typename TorqueLimit>
  static InstructionPacket<protocol_t>
  set_torque_limits(const std::vector<Id> &ids,
                    const std::vector<TorqueLimit> &torque_limits) {
    ...
    for (size_t i = 0; i < torque_limits.size(); i++)
      packed[i] = protocol_t::pack_data(
          (typename ct_t::torque_limit_t)torque_limits[i]);

    return sync_write_t(ct_t::torque_limit,
                        _get_typed<typename protocol_t::id_t>(ids), packed);
  }
