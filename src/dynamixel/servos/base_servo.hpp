...
#define BASE_FIELD(Name)                                                       \
  virtual InstructionPacket<protocol_t> get_##Name() const {                   \
    throw errors::Error("get_" #Name " not implemented in model");             \
                                                                               ...
                                                                               \
  virtual long long int parse_##Name(const StatusPacket<protocol_t> &st)       \
      const {                                                                  \
    throw errors::Error("parse_" #Name " not implemented in model");           \
  }                                                                            \
                                                                               \
  virtual InstructionPacket<protocol_t> set_##Name(long long int value)        \
      const {                                                                  \
    throw errors::Error("set_" #Name " not implemented in model");             \
  }                                                                            \
                                                                               \
  virtual InstructionPacket<protocol_t> reg_##Name(long long int value)        \
      const {                                                                  \
...
template <typename Protocol> class BaseServo {
public:
  ...
  // Common functionality for the servos that will be implemented in the Servo
  // class (where we know the model)
  ...
  virtual double
  parse_present_position_angle(const StatusPacket<protocol_t> &st) const {
    throw errors::Error(
        "parse_present_position_angle not implemented in model");
  }
