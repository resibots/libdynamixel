...
template <class T> class SyncWrite : public InstructionPacket<T> {
public:
  SyncWrite(typename T::address_t address,
            const std::vector<typename T::id_t> &ids,
            const std::vector<std::vector<uint8_t>> &data)
      : InstructionPacket<T>(T::broadcast_id, T::Instructions::sync_write,
                             _get_parameters(address, ids, data)) {}

protected:
  std::vector<uint8_t>
  _get_parameters(uint8_t address, const std::vector<typename T::id_t> &ids,
                  const std::vector<std::vector<uint8_t>> &data) {
    if (ids.size() == 0)
      throw errors::Error("SyncWrite: ids vector of size zero");
    if (ids.size() != data.size())
      throw errors::Error(
          "SyncWrite: mismatching vectors size for ids and data");
  ...
  std::vector<uint8_t>
  _get_parameters(uint16_t address, const std::vector<typename T::id_t> &ids,
                  const std::vector<std::vector<uint8_t>> &data) {
    if (ids.size() == 0)
      throw errors::Error("SyncWrite: ids vector of size zero");
    if (ids.size() != data.size())
